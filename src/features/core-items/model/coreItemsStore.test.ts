...
import { useCoreItemsStore } from './coreItemsStore';
import { useItemStore } from '@/entities/item';
import { useBoardStore } from '@/entities/board';

describe('useCoreItemsStore', () => {
  beforeEach(() => {
  ...
  it('decrements count on normal roll but preserves count on free roll', () => {
    const store = useCoreItemsStore.getState();
  ...
  it('does not repeat the last N rolled numbers', () => {
    useCoreItemsStore.getState().setHistoryConstraintN(3);

    for (let i = 0; i < 20; i++) {
      const historyBefore = [...useCoreItemsStore.getState().rollHistory];
      const roll = useCoreItemsStore.getState().rollRandomNumber(true);
  ...
  it('cycles through all 9 numbers without duplicate in window when N=8', () => {
    useCoreItemsStore.getState().setHistoryConstraintN(8);

    // Roll 9 times
    const rolls: number[] = [];
    for (let i = 0; i < 9; i++) {
      const r = useCoreItemsStore.getState().rollRandomNumber(true);
  ...
  it('generates 3 valid candidate numbers for random choose and advances stream', () => {
    const options1 = useCoreItemsStore.getState().rollRandomChoose(false);
    ...
    const options2 = useCoreItemsStore.getState().rollRandomChoose(true);
  ...
  describe('In-Place Board Tile Rolling & Toggle Check', () => {
    ...
    it('targets tile and confirms choice for Random Choose in Single Use mode', () => {
      ...
      const target = useCoreItemsStore.getState().targetTile;
      expect(target).toEqual({ col: 4, row: 2 });

      const options = useCoreItemsStore.getState().randomChooseOptions;
      ...
      expect(useCoreItemsStore.getState().targetTile).toBeNull();
      expect(useItemStore.getState().isToggled).toBe(false);
      expect(useItemStore.getState().activeItem).toBeNull();
    });
    ...
    it('enforces strict mutual exclusion between items', () => {
      ...
      expect(useCoreItemsStore.getState().targetTile).toEqual({ col: 1, row: 1 });

      // Switching to randomNumber clears targetTile and switches mode
      store.toggleItem('randomNumber');
      expect(useItemStore.getState().activeItem).toBe('randomNumber');
      expect(useCoreItemsStore.getState().targetTile).toBeNull();
      expect(useCoreItemsStore.getState().randomChooseOptions).toBeNull();
    });
    ...
    it('charges randomChoose immediately on tile click and costs an additional item to re-roll', () => {
      ...
      expect(useItemStore.getState().counts.randomChoose).toBe(4);
      expect(useCoreItemsStore.getState().targetTile).toEqual({ col: 0, row: 0 });
      ...
      expect(useCoreItemsStore.getState().targetTile).toEqual({ col: 1, row: 0 });
